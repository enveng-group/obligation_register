...
log() {
  ...
  printf "%s%s - %s%s\n" "$color" "$timestamp" "$message" "$endcolor"
}
...
check_os() {
  if ! [ -f "/etc/debian_version" ]; then
    log "Error: This script is only supported on Debian-based systems." "error"
    exit 1
  fi
}
...
install_pre_reqs() {
  ...
  for cmd in apt-get curl gpg; do
    if ! command_exists "$cmd"; then
      handle_error 1 "Command '$cmd' not found. Please install it and try again."
    fi
  done
  ...
  if ! mkdir -p /usr/share/keyrings; then
    handle_error "$?" "Failed to create directory /usr/share/keyrings"
  fi
...
# Function to configure the NodeSource repository
configure_repo() {
  node_version="$1"
  ...
  if ! apt-get update -y; then
    ...
    log "To install N|solid Runtime, run: apt-get install nsolid -y" "success"
  fi
}
