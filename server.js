...
app.post("/register", async (req, res) => {
  const { fnln, username, email, password, confirm_password, accountability } =
    req.body;
  ...
  try {
    ...
    const query = `
          INSERT INTO auth_user (first_name, last_name, username, email, accountability, password) 
          VALUES (?, ?, ?, ?, ?, ?)`;

    db.run(
      query,
      [first_name, last_name, username, email, accountability, hashedPassword],
      function (err) {
        if (err) {
...
app.get("/responsible", requireLogin, (req, res) => {
    const sql = `SELECT id, username, first_name, last_name, email, accountability FROM auth_user`;
    ...
    db.all(sql, [], (err, rows) => {
      ...
      // Pass the fetched user data and a title to the EJS template
      res.render("responsible", { users: rows, title: "Responsible Users" });
    });
  });
